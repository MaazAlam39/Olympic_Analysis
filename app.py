...
df=pd.read_csv('notebook/athlete_events.csv')
region_df=pd.read_csv('notebook/noc_regions.csv')
...
if user_menu == 'Athlete-wise Analysis':
    ...
    # st.write(f"Total athletes: {athlete_df.shape[0]}")
    ...
    if all([len(x) > 0 for x in [x1, x2, x3, x4]]):
        ...
        st.warning("Insufficient data to create the distribution plot.")

        for name, data in zip(['Overall', 'Gold', 'Silver', 'Bronze'], [x1, x2, x3, x4]):
            if len(data) == 0:
                st.write(f"{name} list is empty")
